...
if(!$cVar_View_Points == 1){KickBackNerd();exit;}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (!empty($_POST["rwid"])) {
		...
		$q7231="UPDATE points_logs SET active = 0 WHERE uid='$did'";
		$qu626231=$cVar_second_db_nfasd8->prepare($q7231);
		$qu626231->execute();
		$q7231="UPDATE points SET points=points-$rwr WHERE pid='$rpid'";
		$qu626231=$cVar_second_db_nfasd8->prepare($q7231);
		$qu626231->execute();
		?>
		<div class="alert_suc alert alert-dismissable alert-danger">
			...
			<h4><strong><?=$d_name ?>'s</strong> Points(<?=$did?>) have been Deleted!</h4> 
			<i>NOTE: All actions are logged!</i>
		</div>
		<?php
		$date2=Date('l jS \of F Y h:i:s A');
		$log="[".$date2."] ".$staffname." has removed ".$rwr." Points from ".$d_name."";
		LogAction(1,$log,$date2);
	}
	if (!empty($_POST["d_id"])) {
		$did=$_POST['d_id'];
	...
	<div class="ts-main-content">
		...
		<div class="content-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						<h2 class="page-title">Warned Users</h2>	
						<form action="warned.php" method="POST">
							<div class="input-group" style="width:300px;">
								<input type="text" id="w_search" name="w_search" class="form-control" placeholder="Search playerid...">
						...
						<i>All users in this list are here because they have broke some form of rule.</i><br>						
						<br>
						<div class="panel panel-default">
							<div class="panel-heading">Warned Users</div>
								...
								<div class="col-md-12">
									<div class="">
										<table id="" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
											...
											<tbody>
												<?php											
												if(!empty($_POST["w_search"]) == 1) { include ("search_results.php"); exit;}
												$urasunt2=$cVar_second_db_nfasd8->prepare("SELECT pid,points FROM points WHERE points > 0");
												$urasunt2->execute();
